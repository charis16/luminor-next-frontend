...
export function getQueryKey(slug: string) {
  return ["album-detail-by-slug", slug];
}

export async function fetchInformation(
  slug: string,
  headers?: Record<string, string>,
): Promise<AlbumDetailBySlugResponse> {
  const res = await goFetcher.get<AlbumDetailBySlugResponse>(
    `/api/album/detail/${slug}`,
    {
      headers,
    },
  );
...
export function getOptions(
  slug: string,
  headers?: Record<string, string>,
): UseQueryOptions<AlbumDetailBySlugResponse, Error> {
  return {
...
// For client
export function useAlbumDetailBySlug(slug: string) {
  return useQuery({
    ...getOptions(slug),
    staleTime: 1000 * 60 * 5,
